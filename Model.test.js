...
check("NPU missing", Model.parseNpuPower(""), null)
check("NPU absent", Model.parseNpuPower('{"present":false,"watts":null}'), { present: false, watts: null })
check("NPU power", Model.parseNpuPower('{"present":true,"watts":2.5}'), { present: true, watts: 2.5 })
check("NPU bad power", Model.parseNpuPower('{"present":true,"watts":-1}'), { present: true, watts: null })
check("NPU allocated from package", Model.cpuComponentW({ cpuW: 10, igpuW: 2 }, { watts: 2.5 }), 5.5)
check("NPU reader absent keeps CPU estimate", Model.cpuComponentW({ cpuW: 10, igpuW: 2 }, null), 8)
